...
| TD-4 | Submodules pinned to commits, not release tags | All eight are pinned at exact commits. Gateways are past tag **`v1.3.1`** (`transcode-gateway` `2a3cbfa` = +6, `openai-gateway` `baf214a` = +4); `open-clearinghouse` is past `v1.3.3` (`98b23ab` = +15); observability repos are untagged at `88f0ff8` / `1c81f00`. Re-pin to release tags where available | 2026-05-26 | Open — pin to release tags when upstream cuts matching releases |
